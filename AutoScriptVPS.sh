...
echo 'echo -e "\e[94m   #+#    #+# #+#    #+# #+#   #+#  #+#          "' >> .bashrc
echo 'echo -e "\e[94m    ########  ###    ### ###    ### ##########   "' >> .bashrc
echo 'echo -e "\e[94m              AutoScriptVPS by  jAkE             "' >> .bashrc
echo 'echo -e "\e[0m"'                                                   >> .bashrc
...
sed -i 's|export KEY_PROVINCE="CA"|export KEY_PROVINCE="OM"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="Calapan"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="JakePrivateVPS"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="jakeprivatevps@gmail.com"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU="MyOrganizationalUnit"|export KEY_OU="VPN"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="AnjVPN"|' /etc/openvpn/easy-rsa/vars
#sed -i 's|export KEY_OU=changeme|export KEY_OU=JakeVPS|' /etc/openvpn/easy-rsa/vars
...
sed -i '10 i # START OPENVPN RULES' /etc/ufw/before.rules
sed -i '11 i # NAT table rules' /etc/ufw/before.rules
sed -i '12 i *nat' /etc/ufw/before.rules
sed -i '13 i :POSTROUTING ACCEPT [0:0]' /etc/ufw/before.rules
sed -i '14 i # Allow traffic from OpenVPN client to eth0' /etc/ufw/before.rules
sed -i '15 i -A POSTROUTING -s 10.3.26.0/24 -o eth0 -j MASQUERADE' /etc/ufw/before.rules
sed -i '16 i COMMIT' /etc/ufw/before.rules
sed -i '17 i # END OPENVPN RULES' /etc/ufw/before.rules
...
echo -e "\e[94m[][][]======================================[][][]"
echo -e "\e[0m                                                   "
echo -e "\e[94m             AutoScriptVPS by  jAkE               "
echo -e "\e[94m                                                  "
